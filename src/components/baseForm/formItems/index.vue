...
export default {
  ...
  methods: {
    ...
    generateTreeSelect(item) {
      return <el-input v-model= { this.formModel[item.name] } onFocus={ (e) => this.showSelectTree(item, e) } style={ item.style || this.defaultStyle } >
    ...
    // 生成日期选择器
    generateDate(item) {
      return <el-date-picker v-model={this.formModel[item.name]} type={item.type} placeholder='选择日期' />
    },
    // 生成颜色选择器
    generateColor(item) {
    ...
    generateFormItems(items = []) {
      ...
      items.forEach((item) => {
        ...
        ele.push(<el-form-item label={item.label} prop={item.name} style={item.formItemStyle} label-width={item.labelWidth} >{ itemEle }</el-form-item>)
      })
      return ele
    }
  },
